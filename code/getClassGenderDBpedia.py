...
def fillListNeedType(list_entities):
  ...
  for entity_name in list_entities:
    if not re.search(' ', entity_name) and not re.search(r'\|', entity_name) and not entity_name.isnumeric() and entity_name[0].isupper() and entity_name not in list_need_type:
